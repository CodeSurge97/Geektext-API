...
class Order(db.Model):
    ...
    def __repr__(self):
        return "Order( orderID: {self.id}, userID: {self.user_id}, date: {self.order_date})"
...
class User(db.Model, UserMixin):
    ...
    def __repr__(self):
        return "User( email: '{self.email}', username: '{self.username}')"
...
class CreditCard(db.Model):
    ...
    def __repr__(self):
        return "CreditCard( card_number: '{self.card_number[:4]}', user: '{self.user.name}')"


# The Comment table stores the comments for each book the attributes of the Comment table are: id, content,
# creation_date, book_isbn, and user_id you can also access the user that made the comment and the book that the
# comment is about because of the relationships.
...
class Cart(db.Model):
    ...
    productId = db.Column(db.Integer, db.ForeignKey('book.isbn'), primary_key=True)
